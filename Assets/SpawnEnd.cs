...
public struct SpawnableItemssss
{
...
public class SpawnEnd : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentSample < 6824563) // Replace `certainNumber` with your specific audio sample threshold
        {
            Origin.SetActive(false);
        }

        if (currentSample > 7445491) // Replace `certainNumber` with your specific audio sample threshold
        {
            Origin.SetActive(false);
        }

        if (currentSample > 6824563 & currentSample < 7445491) // Replace `certainNumber` with your specific audio sample threshold
        {
        ...
        {
            if (Vector3.Distance(carTransform.position, activePrefabs[i].transform.position) > 200f)
            {
                activePrefabs[i].SetActive(false);
                activePrefabs.RemoveAt(i); // Remove the prefab from the list if it's no longer active
            }
        }
    }
